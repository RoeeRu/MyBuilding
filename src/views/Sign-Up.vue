	...
	export default ({
		...
		methods: {
			...
			resgiterNewApi(user) {
						...
						axios.post(process.env.SYSTEM_DOMAIN + '/registration/new', {accessToken: user.accessToken})
				      .then(response => {
			...
			isUserLoggedIn(token) {
				return axios.post(process.env.SYSTEM_DOMAIN + '/registration/isSignedIn', {accessToken: token})
					.then(response => {
